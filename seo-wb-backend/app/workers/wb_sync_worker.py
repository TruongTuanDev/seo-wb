...
class WbSyncWorker:
    def __init__(self) -> None:
        ...
        if not redis_url:
            raise RuntimeError("REDIS_URL or REDIS_HOST/REDIS_PASSWORD is required by the WB sync worker.")
        ...
        self.processor = WbSyncJobProcessor(self.settings, self.redis)
    ...
    def _consume(self) -> None:
        params = pika.URLParameters(self.settings.effective_rabbitmq_url)
    ...
    def _on_message(
        ...
        future.add_done_callback(
            lambda completed: channel.connection.add_callback_threadsafe(
                lambda: self._finish_message(channel, method, properties, body, completed)
            )
        )
    ...
    def _finish_message(
        ...
        if exc is None:
            ...
            return
        if isinstance(exc, RetryableSyncJobError):
            logger.warning("Retryable WB sync error; requeueing. routing_key=%s error=%s", method.routing_key, exc)
            channel.basic_nack(method.delivery_tag, requeue=True)
            return
        if isinstance(exc, InvalidSyncJobError):
            logger.error("Invalid WB sync message; dead-lettering. routing_key=%s error=%s", method.routing_key, exc)
            channel.basic_nack(method.delivery_tag, requeue=False)
            return

        logger.error(
            "WB sync message failed; dead-lettering. routing_key=%s error=%s",
            method.routing_key,
            exc,
