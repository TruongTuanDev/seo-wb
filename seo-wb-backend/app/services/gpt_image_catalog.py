...
def _normalized_focus_crop_box(garment_area: str, pose: str) -> tuple[float, float, float, float] | None:
    ...
    if area == "lower_body":
        boxes = {
            # Lower-body crops must start above the belly/waistband, not at the hips or thighs.
            "crop_front": (0.06, 0.00, 0.94, 0.99),
            "crop_side_45": (0.10, 0.00, 0.98, 0.99),
            "crop_back": (0.06, 0.00, 0.94, 0.99),
        }
        return boxes.get(pose_key, (0.06, 0.00, 0.94, 0.99))
    if area == "upper_body":
        boxes = {
